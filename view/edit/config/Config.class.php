...
class Patchworks_View_Edit_Config extends Action
{
    ...
    function execute()
    {
        // patchworks ID ごとの設定情報を編集する
        $id = intval($this->patchworksView->getPatchworksID($this->block_id));
        if ($id == 0) {
            return "error";
        }
        ...
        $x = BASE_DIR."/extra/addin/patchworksID/".$this->patchworks_id."/patchworks_view_edit_config_".$this->patchworks_id.".html";
        if (!is_file($x)) {
            $x="";
        }
