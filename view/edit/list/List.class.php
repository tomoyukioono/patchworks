...
class Patchworks_View_Edit_List extends Action
{
    ...
    function execute()
    {
        ...
        if (count($x)== 0 ) {
            $x = parse_ini_file(BASE_DIR."/extra/addin/patchworksID/999999/name.ini");
        ...
        foreach ($xarray as $v) {
            ...
            if ($x) {
                ...
                $xx[$v]['name'] = "";
                $x = BASE_DIR."/extra/addin/patchworksID/".$v."/name.ini";
                if (is_file($x)) {
                    $x = parse_ini_file($x);
                ...
                $config->patchworks_description = $xx[$v]['description'];
